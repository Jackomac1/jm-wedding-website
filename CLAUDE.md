...
- **Deployment:** Railway — connected to GitHub repo `Jackomac1/jm-wedding-website`, auto-deploys on push to `main`. Live at `https://majaandjack.ca`. Custom domain configured on Railway; `www.majaandjack.ca` is the primary domain.
- **Persistent storage (fixed 2026-09-20):** `server.js` reads `process.env.DATA_DIR` — when set (to a Railway Volume's mount path, e.g. `/data`), `data/`, `Images/`, and `audio/` all live under it instead of `__dirname`, so they survive redeploys. Falls back to the old `__dirname`-relative paths when `DATA_DIR` is unset (local dev is unaffected). **Before this fix**, any redeploy — a code push, or even just changing a Railway environment variable — wiped `db.json` and every uploaded image/audio file, since Railway's container filesystem is otherwise ephemeral. This bit us twice: once during initial setup (empty guest list, no real cost) and once after go-live with real RSVPs already submitted (required a full manual backup/restore via the admin API). If `DATA_DIR` is ever unset again (Volume removed, env var deleted), this exposure comes back immediately.
